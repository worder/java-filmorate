 ...
 * Проверяет корректность формирования рекомендаций фильмов на основе адаптированного алгоритма Weighted Slope One.
 * Используется встроенная база данных H2 без мок-объектов, с 7 пользователями для тестирования.
 */
@SpringBootTest
...
public class RecommendationServiceTest {
    ...
    public void setUp() {
        ...
        jdbcTemplate.update(
                ...
                1, "user1@example.com", "user1", "User1", LocalDate.of(1990, 1, 1)
        );
        jdbcTemplate.update(
                "INSERT INTO users (id, email, login, name, birthday) VALUES (?, ?, ?, ?, ?)",
                2, "user2@example.com", "user2", "User2", LocalDate.of(1991, 2, 2)
        );
        jdbcTemplate.update(
                "INSERT INTO users (id, email, login, name, birthday) VALUES (?, ?, ?, ?, ?)",
                3, "user3@example.com", "user3", "User3", LocalDate.of(1992, 3, 3)
        );
        jdbcTemplate.update(
                "INSERT INTO users (id, email, login, name, birthday) VALUES (?, ?, ?, ?, ?)",
                4, "user4@example.com", "user4", "User4", LocalDate.of(1993, 4, 4)
        );
        jdbcTemplate.update(
                "INSERT INTO users (id, email, login, name, birthday) VALUES (?, ?, ?, ?, ?)",
                5, "user5@example.com", "user5", "User5", LocalDate.of(1994, 5, 5)
        );
        jdbcTemplate.update(
                "INSERT INTO users (id, email, login, name, birthday) VALUES (?, ?, ?, ?, ?)",
                6, "user6@example.com", "user6", "User6", LocalDate.of(1995, 6, 6)
        );
        jdbcTemplate.update(
                "INSERT INTO users (id, email, login, name, birthday) VALUES (?, ?, ?, ?, ?)",
                7, "user7@example.com", "user7", "User7", LocalDate.of(1996, 7, 7)
        );

        // Добавление 4 фильмов
        jdbcTemplate.update(
                "INSERT INTO films (id, name, description, release_date, duration, mpa_rating_id) VALUES (?, ?, ?, ?, ?, ?)",
                1, "Film1", "Description1", LocalDate.of(2000, 1, 1), 120, 1
        );
        jdbcTemplate.update(
                "INSERT INTO films (id, name, description, release_date, duration, mpa_rating_id) VALUES (?, ?, ?, ?, ?, ?)",
                2, "Film2", "Description2", LocalDate.of(2000, 2, 1), 130, 1
        );
        jdbcTemplate.update(
                "INSERT INTO films (id, name, description, release_date, duration, mpa_rating_id) VALUES (?, ?, ?, ?, ?, ?)",
                3, "Film3", "Description3", LocalDate.of(2000, 3, 1), 140, 1
        );
        jdbcTemplate.update(
                "INSERT INTO films (id, name, description, release_date, duration, mpa_rating_id) VALUES (?, ?, ?, ?, ?, ?)",
                4, "Film4", "Description4", LocalDate.of(2000, 4, 1), 150, 1
        );
    ...
    public void testGetRecommendationsNoLikes() {
        ...
        jdbcTemplate.update(
                "INSERT INTO users (id, email, login, name, birthday) VALUES (?, ?, ?, ?, ?)",
                8, "user8@example.com", "user8", "User8", LocalDate.of(1997, 8, 8)
        );
    ...
    public void testGetRecommendationsUserNotFound() {
        NotFoundException exception = assertThrows(
                ...
                "Должно выброситься исключение NotFoundException"
        );
    ...
    public void testGetRecommendationsRanking() {
        ...
        assertEquals(2, recommendations.size(), "Должно быть две рекомендации");
        assertTrue(recommendations.stream().anyMatch(f -> f.getName().equals("Film1")),
                "Film1 должен быть в рекомендациях (score=3)");
        assertTrue(recommendations.stream().anyMatch(f -> f.getName().equals("Film2")),
                "Film2 должен быть в рекомендациях (score=3)");
    }
    ...
    public void testGetRecommendationsExcludesLikedFilms() {
        ...
        assertFalse(recommendations.stream().anyMatch(f -> f.getName().equals("Film1")),
                "Film1 не должен быть в рекомендациях");
        assertFalse(recommendations.stream().anyMatch(f -> f.getName().equals("Film2")),
                "Film2 не должен быть в рекомендациях");
    }
    ...
    public void testGetRecommendationsPartialOverlap() {
        ...
        assertEquals(2, recommendations.size(), "Должно быть две рекомендации");
        assertTrue(recommendations.stream().anyMatch(f -> f.getName().equals("Film3")),
                "Film3 должен быть в рекомендациях (score=2)");
        assertTrue(recommendations.stream().anyMatch(f -> f.getName().equals("Film2")),
                "Film2 должен быть в рекомендациях (score=2)");
    }
}